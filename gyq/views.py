...
def axis_students(request):  # "1"=新增 "2"="修改" "3"="删除"
    ...
    if(tag == "1"):
        ...
        # pdData = pdData[pdData["xueshengbianhao"] != xueshengbianhao]
        # 将修改后的数据写入JSON文件
...
def student_search(request):
    if request.method == 'POST':
        ...
        file_name1 = os.path.dirname(os.path.abspath('__file__')) + "\\statics\\html\\data\\students.json"
        # 读取students.json文件
        with open(file_name1, 'r', encoding='utf-8') as students_file:
            students_data = json.load(students_file)

        file_name2 = os.path.dirname(os.path.abspath('__file__')) + "\\statics\\html\\data\\exam.json"
        # 读取exam.json文件
        with open(file_name2, 'r', encoding='utf-8') as exam_file:
            exam_data = json.load(exam_file)

            # 根据关联条件查询学生信息
            matched_students = []
            for student in students_data:
                student_id = student.get('xueshengbianhao')
                if student_id in exam_data:
                    word_count = len(exam_data[student_id]['renshidanci'].split())  # 计算单词数量
                    if word_count > 100:
                        student['renshidanci_count'] = word_count  # 添加单词数量到学生数据中
                        matched_students.append(student)

            return render(request, 'myapp/main_1.html', {'students': matched_students})

    return render(request, 'myapp/main_1.html')